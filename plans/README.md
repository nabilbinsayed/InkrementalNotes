...
> **Note on Wave 2026-09-03 (Plans 048–051)**:
> Generated by deep `/improve` audit on 2026-09-03 against HEAD `bd18cc4`.
> Resolves critical viewport geometry & scale explosion (048), UI action card & modal wiring (049),
> high-frequency inking & spatial eraser optimizations (050), and PDF annotation & undo durability (051).

## Dependency & Priority Graph

```mermaid
flowchart TD
    P048["048: Viewport Engine Geometry & Tile Scale Clamping (P0)"]
    P049["049: UI Modal Action Wiring & Focus Trapping (P1)"]
    P050["050: Inking Pipeline & Spatial Eraser Optimization (P1)"]
    P051["051: PDF Annotation Integrity & Undo Durability (P0)"]
...
**Current wave (2026-09-03 audit — plans 048–051):**

1. **Wave 1 — High-Priority Engine & Durability (P0)**:
   - **Plan 048** (Viewport Engine Geometry & Tile Rasterization Scale Clamping): Fixes `stageRect` initialization, horizontal clamping, split-view right pane panning, and boundary tile scale explosion in backend.
   - **Plan 051** (PDF Annotation Integrity & Undo Durability): Fixes 32-hex third-party annotation deletion in `render_tile`, undo/redo IPC synchronization, coordinate normalisation in `pdf.rs`, and "Save As" WAL rebinding.
2. **Wave 2 — Interactive Inking & User Ergonomics (P1)**:
   - **Plan 050** (Inking Pipeline Streamlining & Spatial Eraser): Re-enables `Stroke.push` for One-Euro filtering and jitter deduplication in `pen.js`, batches eraser deletions per gesture, and defers lasso transforms to wet canvas matrices.
   - **Plan 049** (UI Modal Action Wiring & Accessibility): Wires Export & Preferences tabs, implements focus traps and Escape dismissal, fixes inline text formatting buttons, and resolves 'E' spring-hold keyup regression.

**Historical waves (plans 001–047)**: see git history; all marked DONE except 037 accuracy scope.
